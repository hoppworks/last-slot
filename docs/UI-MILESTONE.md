...
`tests/e2e/specs/last-slot.spec.ts` are implemented.
...
4. The two-context Playwright race is green against a fresh Docker Compose
...
- Rust checks, Flutter analysis/tests, TypeScript checks, Docker build, and the
  full Playwright journey all pass from a clean checkout.
